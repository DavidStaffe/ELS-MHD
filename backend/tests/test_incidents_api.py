...
class TestMetaEndpoint:
    ...
    def test_meta_returns_version_0_2_0(self, api_client):
        ...
        data = response.json()
        assert data["version"] == "0.2.0", f"Expected version 0.2.0, got {data.get('version')}"
...
class TestIncidentsListAndFilter:
    ...
    def test_filter_by_status_operativ(self, api_client):
        ...
        response = api_client.get(f"{BASE_URL}/api/incidents", params={"status": "operativ"})
        assert response.status_code == 200
        
        data = response.json()
        assert isinstance(data, list)
        for incident in data:
            assert incident["status"] == "operativ", f"Expected status 'operativ', got {incident.get('status')}"
    ...
    def test_filter_by_demo_true(self, api_client):
        ...
        assert isinstance(data, list)
        for incident in data:
            assert incident["demo"] == True, f"Expected demo=true, got {incident.get('demo')}"
...
class TestDemoIncidentCreation:
    ...
    def test_create_demo_incident(self, api_client):
        ...
        assert data["status"] == "operativ", f"Expected status 'operativ', got {data.get('status')}"
...
class TestIncidentCRUD:
    ...
    def test_create_incident_with_valid_payload(self, api_client):
        ...
        assert "start_at" in data and data["start_at"] is not None, "start_at should be set"
        assert "id" in data
    ...
    def test_patch_incident_status_abgeschlossen_sets_end_at(self, api_client, test_incident_id):
        """PATCH /api/incidents/{id} with status=abgeschlossen should set end_at automatically"""
        payload = {"status": "abgeschlossen"}
        response = api_client.patch(f"{BASE_URL}/api/incidents/{test_incident_id}", json=payload)
        assert response.status_code == 200
        
        data = response.json()
        assert data["status"] == "abgeschlossen"
        assert "end_at" in data, "end_at should be present after closing"
        assert data["end_at"] is not None, "end_at should be set when status is abgeschlossen"
...
class TestIncidentUpdate:
    ...
    def test_update_incident_name(self, api_client):
        ...
        update_resp = api_client.patch(f"{BASE_URL}/api/incidents/{incident_id}", json=update_payload)
