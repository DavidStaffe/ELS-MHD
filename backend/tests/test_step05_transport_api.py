...
class TestMetaVersion:
    ...
    def test_meta_returns_version_050(self, api_client):
        ...
        data = response.json()
        assert data["version"] == "0.5.0", f"Expected version 0.5.0, got {data.get('version')}"
        assert data["step"] == "05 – Transportuebersicht", f"Expected step '05 – Transportuebersicht', got {data.get('step')}"
...
class TestTransportListEndpoint:
    ...
    def test_list_transports_returns_array(self, api_client, test_incident):
        ...
        response = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports")
        assert response.status_code == 200
...
class TestTransportCreate:
    ...
    def test_create_transport_without_ressource_status_offen(self, api_client, test_incident):
        """POST without ressource should create transport with status=offen"""
        payload = {
            "typ": "extern",
            "ziel": "krankenhaus"
        }
        response = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json=payload)
        assert response.status_code == 201, f"Expected 201, got {response.status_code}: {response.text}"
        
        data = response.json()
        assert data["status"] == "offen", f"Expected status 'offen', got {data.get('status')}"
        assert data["ressource"] is None, f"Expected ressource None, got {data.get('ressource')}"
    ...
    def test_create_transport_with_ressource_status_zugewiesen(self, api_client, test_incident):
        """POST with ressource should create transport with status=zugewiesen and zugewiesen_at set"""
        payload = {
            "typ": "intern",
            "ziel": "uhs",
            "ressource": "UHS Team 1"
        }
        response = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json=payload)
        assert response.status_code == 201
        
        data = response.json()
        assert data["status"] == "zugewiesen", f"Expected status 'zugewiesen', got {data.get('status')}"
    ...
    def test_create_transport_with_patient_copies_kennung_sichtung(self, api_client, test_incident):
        """POST with patient_id should copy patient_kennung and patient_sichtung"""
        # Create a patient first
        patient_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S2"})
        ...
        assert data["patient_kennung"] == patient["kennung"], f"Expected kennung {patient['kennung']}, got {data.get('patient_kennung')}"
        assert data["patient_sichtung"] == "S2", f"Expected sichtung S2, got {data.get('patient_sichtung')}"
...
class TestTransportUpdate:
    ...
    def test_patch_ressource_sets_status_zugewiesen(self, api_client, test_incident):
        ...
        create_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "extern",
            "ziel": "krankenhaus"
        })
        ...
        response = api_client.patch(f"{BASE_URL}/api/transports/{transport['id']}", json={"ressource": "RTW 1"})
        assert response.status_code == 200
    ...
    def test_patch_ressource_empty_removes_assignment(self, api_client, test_incident):
        ...
        create_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "intern",
            "ziel": "uhs",
            "ressource": "UHS Team 2"
        })
        transport = create_resp.json()
        assert transport["status"] == "zugewiesen"
        
        # Remove ressource
        response = api_client.patch(f"{BASE_URL}/api/transports/{transport['id']}", json={"ressource": ""})
        assert response.status_code == 200
    ...
    def test_patch_status_unterwegs_sets_gestartet_at(self, api_client, test_incident):
        ...
        response = api_client.patch(f"{BASE_URL}/api/transports/{transport['id']}", json={"status": "unterwegs"})
        assert response.status_code == 200
    ...
    def test_patch_status_abgeschlossen_sets_abgeschlossen_at(self, api_client, test_incident):
        """PATCH with status=abgeschlossen sets abgeschlossen_at"""
        # Create transport with ressource
        create_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "intern",
            "ziel": "uhs",
            "ressource": "UHS Team 3"
        })
        transport = create_resp.json()
        
        # Set status to unterwegs first
        api_client.patch(f"{BASE_URL}/api/transports/{transport['id']}", json={"status": "unterwegs"})
        
        # Set status to abgeschlossen
        response = api_client.patch(f"{BASE_URL}/api/transports/{transport['id']}", json={"status": "abgeschlossen"})
        assert response.status_code == 200
...
class TestAutoCreateTransport:
    ...
    def test_patch_patient_transport_typ_intern_creates_transport(self, api_client, test_incident):
        """PATCH patient with transport_typ=intern automatically creates Transport entry"""
        # Create patient
        patient_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S1"})
        patient = patient_resp.json()
        
        # Set transport_typ
        api_client.patch(f"{BASE_URL}/api/patients/{patient['id']}", json={"transport_typ": "intern"})
        
        # Check transports
        transports_resp = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports")
        transports = transports_resp.json()
        
        # Find transport for this patient
        patient_transports = [t for t in transports if t.get("patient_id") == patient["id"]]
        assert len(patient_transports) >= 1, f"Expected at least 1 transport for patient, found {len(patient_transports)}"
    ...
    def test_patch_patient_transport_typ_extern_creates_transport(self, api_client, test_incident):
        ...
        # Set transport_typ
        api_client.patch(f"{BASE_URL}/api/patients/{patient['id']}", json={"transport_typ": "extern"})
        ...
        assert transport["ziel"] == "rd", f"Expected ziel 'rd' for extern, got {transport.get('ziel')}"
...
class TestAutoCompleteTransport:
    ...
    def test_patch_patient_status_entlassen_completes_transports(self, api_client, test_incident):
        """PATCH patient with status=entlassen sets associated transports to abgeschlossen"""
        # Create patient with transport
        patient_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S3"})
        patient = patient_resp.json()
        api_client.patch(f"{BASE_URL}/api/patients/{patient['id']}", json={"transport_typ": "intern"})
        ...
        transports_resp = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports")
        transports = transports_resp.json()
        patient_transport = next((t for t in transports if t.get("patient_id") == patient["id"]), None)
        assert patient_transport is not None
        assert patient_transport["status"] != "abgeschlossen"
        
        # Set patient status to entlassen
        api_client.patch(f"{BASE_URL}/api/patients/{patient['id']}", json={"status": "entlassen"})
        
        # Check transport is now abgeschlossen
        transport_resp = api_client.get(f"{BASE_URL}/api/transports/{patient_transport['id']}")
        updated_transport = transport_resp.json()
        assert updated_transport["status"] == "abgeschlossen", f"Expected status 'abgeschlossen', got {updated_transport.get('status')}"
    ...
    def test_patch_patient_status_uebergeben_completes_transports(self, api_client, test_incident):
        ...
        api_client.patch(f"{BASE_URL}/api/patients/{patient['id']}", json={"status": "uebergeben"})
...
class TestCascadeDelete:
    ...
    def test_delete_incident_removes_transports(self, api_client):
        ...
        # Create incident
        incident_resp = api_client.post(f"{BASE_URL}/api/incidents", json={
        ...
        # Create transport
        transport_resp = api_client.post(f"{BASE_URL}/api/incidents/{incident['id']}/transports", json={
            "typ": "extern",
            "ziel": "krankenhaus"
        })
...
class TestTransportFiltering:
    ...
    def test_filter_by_typ_intern(self, api_client, test_incident):
        ...
        intern_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "intern",
            "ziel": "uhs"
        })
        extern_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "extern",
            "ziel": "krankenhaus"
        })
        ...
        response = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports?typ=intern")
        assert response.status_code == 200
    ...
    def test_filter_by_status_offen(self, api_client, test_incident):
        ...
        offen_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "extern",
            "ziel": "krankenhaus"
        })
        zugewiesen_resp = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports", json={
            "typ": "intern",
            "ziel": "uhs",
            "ressource": "UHS Team 1"
        })
        ...
        response = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/transports?status=offen")
        assert response.status_code == 200
        
        data = response.json()
        for t in data:
            assert t["status"] == "offen", f"Expected status 'offen', got {t.get('status')}"
