...
class TestMetaVersion:
    ...
    def test_meta_returns_version_040(self, api_client):
        ...
        assert (
            data["version"] == "0.4.0"
        ), f"Expected version 0.4.0, got {data.get('version')}"
        assert (
            data["step"] == "04 – Patientendetail"
        ), f"Expected step '04 – Patientendetail', got {data.get('step')}"
...
class TestS4ToS0Migration:
    ...
    def test_post_with_sichtung_s4_returns_422(self, api_client, test_incident):
        ...
        ), f"Expected 422 for S4, got {response.status_code}: {response.text}"
    ...
    def test_post_with_sichtung_s0_succeeds(self, api_client, test_incident):
        ...
        assert (
            response.status_code == 201
        ), f"Expected 201 for S0, got {response.status_code}: {response.text}"

        data = response.json()
        assert (
            data["sichtung"] == "S0"
        ), f"Expected sichtung 'S0', got {data.get('sichtung')}"
        assert (
            data["status"] == "in_behandlung"
        ), f"Expected status 'in_behandlung', got {data.get('status')}"
    ...
    def test_no_s4_patients_in_database(self, api_client, test_incident):
        ...
        for patient in data:
            assert (
                patient.get("sichtung") != "S4"
            ), f"Found patient with S4: {patient.get('kennung')}"


class TestTransportTyp:
    """Tests for transport_typ field and automatic status progression"""

    def test_patch_transport_typ_intern_sets_status_and_timestamp(
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}", json={"transport_typ": "intern"}
        )
        assert response.status_code == 200

        data = response.json()
        assert (
            data["transport_typ"] == "intern"
        ), f"Expected transport_typ 'intern', got {data.get('transport_typ')}"
        assert (
            data["status"] == "transportbereit"
        ), f"Expected status 'transportbereit', got {data.get('status')}"
        assert (
...
class TestBehandlungRessource:
    """Tests for behandlung_ressource_id assignment on patients"""

    def test_patch_assigns_behandlung_ressource_name(self, api_client, test_incident):
        p = api_client.post(
            f"{BASE_URL}/api/incidents/{test_incident['id']}/patients",
            json={"sichtung": "S2"},
        ).json()
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"behandlung_ressource_id": r["id"]},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["behandlung_ressource_id"] == r["id"]
        assert data["behandlung_ressource_name"] == "EVT 1"

        api_client.delete(f"{BASE_URL}/api/patients/{p['id']}")
        api_client.delete(f"{BASE_URL}/api/resources/{r['id']}")

    def test_patch_rejects_unknown_behandlung_ressource(
        self, api_client, test_incident
    ):
        p = api_client.post(
            f"{BASE_URL}/api/incidents/{test_incident['id']}/patients",
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"behandlung_ressource_id": "does-not-exist"},
        )
        assert response.status_code == 422

        api_client.delete(f"{BASE_URL}/api/patients/{p['id']}")

    def test_resource_changes_append_timeline_events(self, api_client, test_incident):
        p = api_client.post(
            f"{BASE_URL}/api/incidents/{test_incident['id']}/patients",
            json={"sichtung": "S2"},
        ).json()
        ...
        d1 = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"behandlung_ressource_id": r1["id"]},
        ).json()
        d2 = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"behandlung_ressource_id": r2["id"]},
        ).json()
        d3 = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"behandlung_ressource_id": None},
        ).json()

        e1 = d1.get("behandlung_ressource_events") or []
        e2 = d2.get("behandlung_ressource_events") or []
        e3 = d3.get("behandlung_ressource_events") or []

        assert len(e1) == 1
        assert e1[-1]["action"] == "assigned"
        assert e1[-1].get("ts")

        assert len(e2) == 2
        assert e2[-1]["action"] == "changed"
        assert e2[-1].get("from_name") == "EVT 1"
        assert e2[-1].get("to_name") == "EVT 2"
        assert e2[-1].get("ts")

        assert len(e3) == 3
        assert e3[-1]["action"] == "cleared"
        assert e3[-1].get("from_name") == "EVT 2"
        assert e3[-1].get("to_name") is None
        assert e3[-1].get("ts")

        api_client.delete(f"{BASE_URL}/api/patients/{p['id']}")
        api_client.delete(f"{BASE_URL}/api/resources/{r1['id']}")
        api_client.delete(f"{BASE_URL}/api/resources/{r2['id']}")

    def test_patch_transport_typ_extern_sets_status_and_timestamp(
        self, api_client, test_incident
    ):
        ...
        # Update with transport_typ
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}", json={"transport_typ": "extern"}
        )
        assert response.status_code == 200
...
class TestFallabschlussTyp:
    ...
    def test_patch_fallabschluss_rd_uebergabe(self, api_client, test_incident):
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}",
            json={"fallabschluss_typ": "rd_uebergabe"},
        )
        assert response.status_code == 200

        data = response.json()
        assert (
            data["fallabschluss_typ"] == "rd_uebergabe"
        ), f"Expected fallabschluss_typ 'rd_uebergabe', got {data.get('fallabschluss_typ')}"
        assert (
            data["status"] == "uebergeben"
        ), f"Expected status 'uebergeben', got {data.get('status')}"
        assert (
            data["verbleib"] == "rd"
        ), f"Expected verbleib 'rd', got {data.get('verbleib')}"
    ...
    def test_patch_fallabschluss_manuell(self, api_client, test_incident):
        ...
        # Update with fallabschluss_typ
        response = api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}", json={"fallabschluss_typ": "manuell"}
        )
        assert response.status_code == 200
        ...
        ), f"Expected verbleib 'unbekannt' for manuell, got {data.get('verbleib')}"
        assert data["fallabschluss_at"] is not None
    ...
    def test_get_patient_includes_fallabschluss_typ(self, api_client, test_incident):
        ...
        data = response.json()
        assert (
            "fallabschluss_typ" in data
        ), "fallabschluss_typ field should be in response"
        assert data["fallabschluss_typ"] == "entlassung"
    ...
    def test_fallabschluss_preserves_existing_verbleib(self, api_client, test_incident):
        ...
        api_client.patch(
            f"{BASE_URL}/api/patients/{p['id']}", json={"verbleib": "krankenhaus"}
        )
        ...
        assert (
            data["verbleib"] == "krankenhaus"
        ), f"Expected verbleib 'krankenhaus' to be preserved, got {data.get('verbleib')}"
...
class TestSmartStatusProgression:
    ...
    def test_sichtung_patch_sets_timestamps_but_not_status(
        self, api_client, test_incident
    ):
...
class TestAllSichtungLevels:
    ...
    def test_create_patient_with_valid_sichtung(
        self, api_client, test_incident, sichtung
    ):
        ...
        assert (
            response.status_code == 201
        ), f"Expected 201 for {sichtung}, got {response.status_code}"
