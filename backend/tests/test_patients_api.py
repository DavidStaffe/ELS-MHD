...
class TestPatientCreation:
    ...
    def test_create_patient_without_payload_defaults(self, api_client, test_incident):
        ...
        response = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={})
        assert response.status_code == 201, f"Expected 201, got {response.status_code}: {response.text}"
        
        data = response.json()
        # Verify defaults
        assert data["status"] == "wartend", f"Expected status 'wartend', got {data.get('status')}"
        assert data["kennung"].startswith("P-"), f"Kennung should start with P-, got {data.get('kennung')}"
        assert len(data["kennung"]) == 6, f"Kennung should be P-XXXX format, got {data.get('kennung')}"
        assert data["sichtung"] is None, f"Sichtung should be None by default, got {data.get('sichtung')}"
        assert data["verbleib"] == "unbekannt", f"Verbleib should be 'unbekannt', got {data.get('verbleib')}"
    ...
    def test_create_patient_with_sichtung_s1(self, api_client, test_incident):
        ...
        data = response.json()
        assert data["sichtung"] == "S1", f"Expected sichtung 'S1', got {data.get('sichtung')}"
        assert data["status"] == "in_behandlung", f"Expected status 'in_behandlung' when sichtung set, got {data.get('status')}"
        assert data["sichtung_at"] is not None, "sichtung_at should be set when sichtung provided"
        assert data["behandlung_start_at"] is not None, "behandlung_start_at should be set when sichtung provided"
    ...
    def test_create_patient_invalid_sichtung_returns_422(self, api_client, test_incident):
        """POST with invalid sichtung (e.g., 'SX') should return 422"""
        payload = {"sichtung": "SX"}
        response = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json=payload)
        assert response.status_code == 422, f"Expected 422 for invalid sichtung, got {response.status_code}"
    
    def test_create_patient_invalid_incident_returns_404(self, api_client):
        """POST to non-existent incident should return 404"""
        fake_id = str(uuid.uuid4())
        response = api_client.post(f"{BASE_URL}/api/incidents/{fake_id}/patients", json={})
...
class TestPatientAutoKennung:
    ...
    def test_auto_kennung_sequential(self, api_client, test_incident):
        ...
        resp1 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={})
        assert resp1.status_code == 201
        p1 = resp1.json()
        kennung1 = p1["kennung"]
        
        # Create second patient
        resp2 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={})
        ...
        assert num2 == num1 + 1, f"Kennung should be sequential: {kennung1} -> {kennung2}"
...
class TestPatientList:
    ...
    def test_list_patients_filter_by_sichtung(self, api_client, test_incident):
        ...
        p1 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S1"}).json()
        p2 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S2"}).json()
        p3 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S3"}).json()
        
        # Filter by S1,S2
        response = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", params={"sichtung": "S1,S2"})
        assert response.status_code == 200
        
        data = response.json()
        for patient in data:
            assert patient["sichtung"] in ["S1", "S2"], f"Filter failed: got sichtung {patient.get('sichtung')}"
    ...
    def test_list_patients_filter_by_status(self, api_client, test_incident):
        ...
        p1 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S2"}).json()
        # Create patient without sichtung (will have status=wartend)
        p2 = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={}).json()
        
        # Filter by in_behandlung
        response = api_client.get(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", params={"status": "in_behandlung"})
        assert response.status_code == 200
        
        data = response.json()
        for patient in data:
            assert patient["status"] == "in_behandlung", f"Filter failed: got status {patient.get('status')}"
...
class TestPatientUpdate:
    ...
    def test_update_status_transportbereit_sets_timestamp(self, api_client, test_incident):
        ...
        response = api_client.patch(f"{BASE_URL}/api/patients/{p['id']}", json={"status": "transportbereit"})
        assert response.status_code == 200
    ...
    def test_update_status_entlassen_sets_fallabschluss(self, api_client, test_incident):
        ...
        # Update to entlassen
        response = api_client.patch(f"{BASE_URL}/api/patients/{p['id']}", json={"status": "entlassen"})
        assert response.status_code == 200
        
        data = response.json()
        assert data["status"] == "entlassen"
        assert data["fallabschluss_at"] is not None, "fallabschluss_at should be set when entlassen"
        
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/patients/{p['id']}")
    
    def test_update_status_uebergeben_sets_fallabschluss(self, api_client, test_incident):
        ...
        response = api_client.patch(f"{BASE_URL}/api/patients/{p['id']}", json={"status": "uebergeben"})
        assert response.status_code == 200
        
        data = response.json()
        assert data["status"] == "uebergeben"
        assert data["fallabschluss_at"] is not None, "fallabschluss_at should be set when uebergeben"
        
        # Cleanup
        api_client.delete(f"{BASE_URL}/api/patients/{p['id']}")
    
    def test_update_sichtung_on_unsichtiert_patient_sets_timestamps(self, api_client, test_incident):
        ...
        data = response.json()
        assert data["sichtung"] == "S2"
        assert data["sichtung_at"] is not None, "sichtung_at should be set when sichtung added"
        assert data["behandlung_start_at"] is not None, "behandlung_start_at should be set when sichtung added"
    ...
    def test_update_patient_not_found(self, api_client):
        ...
        fake_id = str(uuid.uuid4())
        response = api_client.patch(f"{BASE_URL}/api/patients/{fake_id}", json={"status": "wartend"})
        assert response.status_code == 404
...
class TestIncidentDeleteCascade:
    ...
    def test_delete_incident_removes_patients(self, api_client):
        ...
        # Create incident
        inc_resp = api_client.post(f"{BASE_URL}/api/incidents", json={
        ...
        # Create patients
        p1 = api_client.post(f"{BASE_URL}/api/incidents/{incident['id']}/patients", json={"sichtung": "S1"}).json()
        p2 = api_client.post(f"{BASE_URL}/api/incidents/{incident['id']}/patients", json={"sichtung": "S2"}).json()
        ...
        assert api_client.get(f"{BASE_URL}/api/patients/{p1['id']}").status_code == 404, "Patient should be deleted with incident"
        assert api_client.get(f"{BASE_URL}/api/patients/{p2['id']}").status_code == 404, "Patient should be deleted with incident"
...
class TestPatientGet:
    ...
    def test_get_patient_by_id(self, api_client, test_incident):
        ...
        # Create patient
        p = api_client.post(f"{BASE_URL}/api/incidents/{test_incident['id']}/patients", json={"sichtung": "S2", "notiz": "Test note"}).json()
