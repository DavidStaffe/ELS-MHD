...
## Changelog
- **2026-05-16 (Abschnitt-Delete-Regel + ResourceList-Refactor + Versions-Cleanup)**: Drei zusammenhaengende Aufgaben in einer Iteration. (1) **Abschnitt loeschbar bei leeren Betten**: `routes/abschnitte.delete_abschnitt` umgebaut: alte Regel 'nur loeschbar wenn Incident abgeschlossen' entfernt; neue Regel zaehlt `db.betten.count_documents(abschnitt_id, status='belegt')` → 409 bei occupied>0 mit Detail-Text, sonst 204 (Resources+freie/gesperrte Betten werden mit-entkoppelt). Frontend `AbschnittList.canDelete` vereinfacht auf nur `can('abschnitt.delete')` (Backend ownt die Pruefung). Title-Hinweis am Loesch-Button aktualisiert. Veralteter Test `test_delete_active_incident_returns_409` → `test_delete_active_incident_returns_204` umgestellt. (2) **DRY-Refactor ResourceList**: Neue `ResourceSection`-Komponente in `pages/ResourceList.jsx` extrahiert (Props: title, testId, emptyLabel, resources, abschnittById, abschnitte, vehicles + 6 Handler + 2 Permission-Flags). Beide SectionCard-Bloecke (Intern + Extern) durch jeweils einen `<ResourceSection>`-Aufruf ersetzt → ~50 LOC Duplikat-Code entfernt, gleiches DOM und Funktionalitaet (testids unveraendert). (3) **Versions-Asserts auf 1.0.0 umgestellt**: 4 Tests (`test_step04_api`, `test_step05_transport_api`, `test_step06_resources_messages_konflikte`, `test_incidents_api`) erwarten jetzt `/api/meta` → version=1.0.0 + generischer Step-Format-Check. Testing: 4/4 Backend Delete-Rule + 4/4 Version-Asserts + 2/2 Frontend Delete-Flows + 2/2 ResourceSection-Render gruen (iteration_24). Keine Regressionen.
- **2026-05-16 (Karten-Sidebar Reihenfolge + Resource-Kuerzel + voller FMS-Verlauf im Funktagebuch)**: User-Wunsch komplett umgesetzt. (1) KartePage Sidebar-Reihenfolge: FmsAlertSidebarPanel (oben, bei Alarmen) → Divera 24/7 → FMS-Legende → FMS-Verlauf (limit=5) + Link 'Vollstaendiger Verlauf im Funktagebuch →' (data-testid='map-fms-history-fulllink') → Abschnitte → Nicht platziert (NUR sichtbar wenn unplacedResources.length>0). (2) Funktagebuch bekommt neue collapsible Sektion `FmsHistoryCollapsible` (data-testid='funk-fms-history') ueber der Nachrichten-Liste, default ausgeklappt, limit=500. Toggle-Button data-testid='funk-fms-history-toggle'. (3) Resource-Kuerzel: Backend models.py ResourceBase + ResourceUpdate haben `kuerzel: Optional[str] = Field(default=None, max_length=4)`. **BUGFIX**: routes/resources.py.create_resource liest `data.get('kuerzel')` (vorher gar nicht ausgelesen → persistierte als null). Frontend ResourceList.jsx ResourceDialog hat neues Input-Feld 'Kuerzel max 4' (data-testid='resource-kuerzel-input') neben dem Name-Feld, font-mono, automatisch uppercase, slice(0,4) defensiv. IncidentMap.jsx Marker bevorzugt `r.kuerzel` als Label, fallback auf `r.name.split(' ')[0]`. Testing: 6/6 neue pytest + 5/5 Frontend gruen (iteration_23).
- **2026-05-16 (Pflicht-Identifikation: Vorname+Nachname bei Rollenauswahl)**: User-Wunsch: 'Bei Rollenauswahl Vor-/Nachname als Pflichtfeld. Nutzung im Funktagebuch fuer Dokumentation+Nachvollziehbarkeit'. RoleContext bekommt zusaetzlich `userName` (localStorage Key `els-user-name`) + `setIdentity(role,name)` + `displayName`-Computed. RoleSelector komplett umgebaut zu 'Anmeldung'-Dialog mit Vor-/Nachname-Pflichtfeldern (>=2 Zeichen), 5 Rollen-Karten, disabled Confirm bis alle 3 Werte gueltig. Picker oeffnet sich automatisch wenn ROLE ODER NAME fehlt. GlobalHeader zeigt 'Max Mustermann' + Rolle-Kurz ('EL'). Funktagebuch.handleSave: `erfasst_von = '${userName} (${roleMeta.kurz})'`. Ack-Pfad: `FmsAcknowledgePayload.name: Optional[str]` (max 120), `acknowledge_fms_event(event_id, role, name)` normalisiert Whitespace→None und persistiert `acknowledged_by_name`. FmsHistory zeigt 'quittiert von <Name> (<Rolle>)'. Backward-compat: Calls ohne `name` setzen Feld auf null. Testing: 36/36 Backend (6 NEU + 30 Regression) + 6/6 Frontend (iteration_22).
- **2026-05-16 (Quittierung schickt 'SPRECHEN SIE' an Divera + Status-Revert)**: User-Wunsch: 'Quittieren soll ueber Divera-Messenger SPRECHEN SIE zurueckschicken und FMS auf den vorherigen Status zuruecksetzen'. Komplett umgesetzt. Backend: (1) Neue Funktion `services/divera.set_vehicle_status(divera_id, status_id, status_note)` → GET https://divera247.com/api/fms (Divera-Spec) mit Params accesskey/vehicle_id/status_id/status_note. (2) `services/fms_audit.acknowledge_fms_event()` komplett umgebaut: nach Ack prueft if `from_fms in {1,2,3,4,6,7,8,9}` UND `divera_id` gesetzt → ruft `set_vehicle_status` mit status_note='SPRECHEN SIE' auf. Lokale Resource wird **immer** auf `from_fms` + Status-Mapping zurueckgesetzt (auch wenn Divera-Call scheitert — Resilienz). Event-Doc bekommt 3 neue Felder: `reverted_to_fms`, `revert_sent_to_divera`, `revert_divera_error`. Zusatzlicher SSE-Push `kind='resource', action='fms_reverted'` triggert sofortiges Map-Update. Frontend: FmsHistory zeigt unter quittierten Alerts 'FMS X wiederhergestellt' + Sky-Badge '→ SPRECHEN SIE' wenn erfolgreich, sonst amber 'Divera-Push fehlgeschlagen'. Testing: 14/14 neue pytest (test_fms_revert.py) + 15/15 Ack-Regression gruen (iteration_21).
