...
function DauerSeitSichtung({ patient }) {
    ...
    if (!patient.sichtung_at) return <span className="text-muted-foreground">–</span>;
...
export default function PatientList() {
    ...
    const filtered = React.useMemo(() => {
        ...
        return patients.filter((p) => {
            ...
            // Query
            if (q) {
                const blob = `${p.kennung} ${p.notiz || ""} ${p.verbleib || ""} ${p.status || ""}`.toLowerCase();
    ...
    const columns = [
        ...
        },
        {
            ...
            width: "18%",
            render: (p) => {
                const meta = PATIENT_STATUS[p.status] || { label: p.status, tone: "neutral" };
                return (
                    <StatusBadge
                        tone={meta.tone}
                        variant="soft"
        ...
        },
        {
            ...
            width: "14%",
            render: (p) => (
                <span className={p.verbleib === "unbekannt" ? "text-muted-foreground" : ""}>
