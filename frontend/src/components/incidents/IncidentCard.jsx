...
export function IncidentCard({
  ...
  const duration = React.useMemo(() => {
    if (incident.status === 'geplant' || !incident.start_at) {
      return 'nicht gestartet';
    }
    const end = incident.end_at ? new Date(incident.end_at) : now;
    const start = new Date(incident.start_at);
    return formatDuration(end - start);
  }, [incident.status, incident.start_at, incident.end_at, now]);

  const isClosed =
    incident.status === 'abgeschlossen' || incident.status === 'archiviert';
  const isArchive = variant === 'archive';
  // Show delete: explicit prop override for archive; default = demo or planned
  const canShowDelete =
    showDelete !== undefined
      ? showDelete
      : incident.demo || incident.status === 'geplant';
    ...
    >
      ...
      <div className="flex-1 space-y-2 p-4">
        <div className="flex items-center gap-2 text-body">
          ...
          <span className="truncate">
            {incident.ort || (
              <span className="italic text-muted-foreground">kein Ort</span>
            )}
          </span>
        </div>
        <div className="flex items-center gap-2 text-body">
        ...
        <div className="flex items-center gap-2 text-body">
          ...
          <span className="text-caption text-muted-foreground">Dauer</span>
          <span
      ...
      <div className="flex items-center justify-between gap-2 border-t border-border bg-surface-sunken px-3 py-2">
        <div className="flex items-center gap-1">
          ...
          {!isArchive && incident.status === 'geplant' && (
            <Button
              type="button"
              size="sm"
              variant="ghost"
              onClick={() => (onSetOperativ || onReopen)?.(incident)}
              data-testid={`incident-operativ-${incident.id}`}
            >
              <RotateCcw className="h-3.5 w-3.5" />
              Auf Operativ
            </Button>
